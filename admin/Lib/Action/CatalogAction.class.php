...
class CatalogAction extends BaseAction
{
    ...
    function index()
    {
        $article_cate_mod = M('catalog');
        $article_mod = D('article');
    	$result = $article_cate_mod->order('sort ASC')->select();
...
//    	    	$val['nums'] =$nums;
		...
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=Catalog&a=add\', title:\''.L('add_cate').'\', width:\'700\', height:\'400\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('add_cate'));
    ...
    private function _cate_exists($name, $pid, $id=0)
    {
        ...
        $result = M('catalog')->where($where)->count();
