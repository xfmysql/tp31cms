...
class ListAction extends CommonAction
{
	Public function index(){
		...
		import('ORG.Util.Page');// 导入分页类
   		$count=M('article')->where(" (pubtime is null or pubtime<CURRENT_TIMESTAMP() ) and pid in(select id from cms_catalog where id=$catsid or pid=$catsid) and status=1")->count();//获取数据的总
		...
		$articles=M('article')->where(" (pubtime is null or pubtime<CURRENT_TIMESTAMP() ) and pid in(select id from cms_catalog where id=$catsid or pid=$catsid) and status=1")->
